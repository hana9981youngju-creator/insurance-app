...
st.write("보험 가입현황표 캡처 이미지를 업로드하면 전문적인 분석 리포트를 생성합니다.")

api_key = st.sidebar.text_input("Gemini API Key를 입력하세요", type="password")
st.sidebar.markdown("[👉 API Key 무료 발급받기](https://aistudio.google.com/apikey)")

uploaded_file = st.file_uploader("가입현황표 이미지를 올려주세요 (JPG, PNG)", type=["png", "jpg", "jpeg"])
...
if uploaded_file is not None:
    ...
    if st.button("🚀 보장분석 리포트 생성하기", type="primary"):
        if not api_key:
            ...
            with st.spinner("AI가 보험 가입 내역을 정밀 판독하고 있습니다... (약 10~15초 소요)"):
                try:
                    ...
                    첨부된 보험 가입현황표 이미지를 정밀 분석하여 아래 양식에 맞춘 마크다운 리포트를 작성해주세요.

                    [리포트 출력 형식]
                    ## 📊 1. 종합 총평 및 핵심 요약
                    - 고객 기본 정보 (성함, 연령, 성별, 총 보험료)
